...
const generateFormThreePDF = (data, callback) => {
  try {
    ...
    const pdfPath = path.join(__dirname, `../templates/Form3_${Date.now()}.pdf`);
    const writeStream = fs.createWriteStream(pdfPath);
    doc.pipe(writeStream);
    ...
    doc.moveDown(1);
    doc.fontSize(12).text("APPLICATION CUM DECLARATION FORM FOR EMPANELMENT AS A PEER REVIEWER", {
      align: "center",
    });
    ...
    doc.text("The Institute of Chartered Accountants of India,", { align: "left" });
    ...
    doc.moveDown(1);
    doc.text(`1. I ${data.memberName || "____________________"}, M. No. ${data.memberNo || "________"} would like to apply for Empanelment as a Peer Reviewer.`, { align: "justify" });
    doc.moveDown(1);
    doc.text(`2. I have attended the training Programme organized by the Board physically/ through VCM on ${data.trainingDate || "______________"}.`, { align: "justify" });
    doc.moveDown(1);
    doc.text("3. I have gone through the Peer Review Guidelines 2022 hosted at:");
     doc
      .fillColor("blue")
       .text(
         "https://resource.cdn.icai.org/72010prb57960-peer-review-guidelines-2022.pdf",
   ...
   //
   doc.moveDown(1);
doc.font("Helvetica-Bold").fontSize(14).text("Other information is as follows:", { align: "center" });
doc.moveDown(1);
...
doc.text(`4. Mobile No.: (as per ICAI records)`, labelX, labelY, { align: "left" });
...
for (let i = 0; i < 10; i++) {
  doc.rect(mobileX + (i * (mobileBoxSize + mobileSpacing)), mobileY, mobileBoxSize, mobileBoxSize).stroke();
...
const addressWidth = doc.page.width - doc.page.margins.left - doc.page.margins.right; // Correct width
...
doc.text(data.address || "", addressX + 6, addressY + 5, { width: addressWidth - 15 });
...
const emailBoxSize = 18; // Box width & height
const emailSpacing = 3;  // Space between boxes

// Draw 18 boxes for email input (proper left alignment)
for (let i = 0; i < 18; i++) {
  doc.rect(emailX + (i * (emailBoxSize + emailSpacing)), emailY, emailBoxSize, emailBoxSize).stroke();
...
doc.text("7. Telephone (Landline) Number (optional):", doc.page.margins.left, doc.y);
doc.moveDown(1);
...
for (let i = 0; i < 10; i++) {
  doc.rect(landlineX + (i * (landlineBoxSize + landlineSpacing)), landlineY, landlineBoxSize, landlineBoxSize).stroke();

  // Fill boxes with landline number if provided
  if (data.landline && data.landline[i]) {
    doc.text(
      data.landline[i], 
      landlineX + (i * (landlineBoxSize + landlineSpacing)) + (landlineBoxSize / 3), 
...
for (let i = 0; i < dateFormat.length; i++) {
  ...
  if (dateFormat[i] !== "/") {
    doc.rect(dateCopX + (i * (dateBoxSize + dateSpacing)), dateCopY, dateBoxSize, dateBoxSize).stroke();
    ...
    doc.text("/", dateCopX + (i * (dateBoxSize + dateSpacing)) + (dateBoxSize / 3), dateCopY + (dateBoxSize / 4));
  }
}
...
doc.moveDown(2);
doc.text("9. No. of years Audit and Assurance service experience in Practice:", doc.page.margins.left, doc.y);
...
for (let i = 0; i < 4; i++) {
  doc.rect(auditExpX + (i * (auditBoxSize + auditSpacing)), auditExpY, auditBoxSize, auditBoxSize).stroke();
...
headers.forEach((header, index) => {
  doc.text(header, colX + 5, tableY + 5, { width: colWidths[index], align: "center" });
...
// Draw header row box
doc.rect(tableX, tableY, colWidths.reduce((a, b) => a + b, 4), rowHeight).stroke();
...
for (let i = 0; i < numRows; i++) {
  ...
  if (data.firms && data.firms[i]) {
    ...
    rowData.forEach((text, index) => {
      doc.text(text.toString(), colX + 5, rowY + 7, { width: colWidths[index], align: "center" });
      colX += colWidths[index];
    });
  }
}
...
doc.moveDown(10);
doc.font("Helvetica");
doc.text("11. (I) Have you experience of statutory audit of:", doc.page.margins.left);
...
doc.moveDown(2);
doc.text("(II) If Yes, provide the following details:", doc.page.margins.left);
// doc.moveDown(1);
...
for (let i = 0; i < totalRows; i++) {
  ...
  for (let j = 0; j < totalColumns; j++) {
    doc.rect(currentX, rowYPosition, columnWidths[j], tableRowHeight).stroke();
  ...
  if (data.statAuditRecords && data.statAuditRecords[i]) {
    ...
    rowValues.forEach((text, index) => {
      doc.text(text.toString(), currentX + 5, rowYPosition + 7, { width: columnWidths[index], align: "center" });
...
doc.font("Helvetica");
doc.text("12. Number of years of experience for Audit & Assurance services as a member of the team, while working in employment:", doc.page.margins.left);
...
doc.rect(expYearBoxX + expBoxSize, expBoxY, expBoxSize, expBoxSize).stroke();
...
doc.rect(expMonthBoxX + expBoxSize, expBoxY, expBoxSize, expBoxSize).stroke();
...
const colSizes = [40, 120, 150, 100, 100, 100]; // Column widths for 6 columns
...
// Draw header row box
doc.strokeColor("black");
doc.rect(startX, startY, colSizes.reduce((a, b) => a + b, 0), cellHeight).stroke();
...
for (let i = 0; i < rowCount; i++) {
  ...
  if (data.employmentData && data.employmentData[i]) {
    ...
    doc.font("Helvetica").fillColor("black");
    rowValues.forEach((text, index) => {
      doc.text(text.toString(), positionX + 5, positionY + 8, { width: colSizes[index], align: "center" });
      positionX += colSizes[index];
    });
  }
}
...
doc.moveDown(2);
    ...
    // Declaration Section
    doc.fontSize(12).font("Helvetica-Bold").text("Annexure: Declaration", { align: "left" });
    doc.moveDown(1);
    ...
    doc.text(`Name: ${data.memberName || "____________________"}`, { align: "left" });
    doc.text(`Date: ${data.declarationDate || "__/__/____"}`, { align: "left" });
